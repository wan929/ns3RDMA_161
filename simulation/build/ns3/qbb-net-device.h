...
public:
  ...
  static uint32_t m_RecMode;
  static uint32_t m_CcMode;
	int m_qlast;
	uint32_t m_rrlast;
...
public:
  ...
  //void AddSeqHeader (Ptr<Packet> p, uint16_t seqnumber); //seq header
  //bool ProcessSeqHeader (Ptr<Packet> p, uint16_t& seqparam);
...
//in-network notification
  void SendNotifyToUp(uint16_t left, uint16_t right);
  Ptr<Packet> ProduceEmptyP();
  Ptr<Packet> ProduceEmptyAckP();
  bool NearIsSwitch(Ptr<QbbChannel> channel);

  void SendPauseToUp(uint16_t lgtype, uint8_t qindex);
...
protected:
  ...
  bool TramsmitStartToRecirculate (Ptr<Packet> p);
  
  virtual void DoDispose(void);

  /// Reset the channel into READY state and try transmit again
  virtual void TransmitComplete(void);
  virtual void TransmitCompleteToRecirculate(void);

  void EnqueueReorderingRecirculateQueue(Ptr<Packet> p, uint16_t port);
...
public:
  ...
  //void AddSeq (Ptr<Packet> p, uint16_t seqnumber);
