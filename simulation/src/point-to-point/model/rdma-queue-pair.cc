 ...
 * RdmaQueuePair
 *************************/
Time RdmaQueuePair::m_timeout = MilliSeconds(1);
...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport){
	...
	halt_state = false;
	probe_state = false;
	halt_time_expired = false;
	deduced_seq = 0;
	// for(uint32_t i =0; i<10000; i++)
	// {
	// 	round_seq[i] = 0xffffffff;
	// 	round_vaild[i] = 1;
	// }
...
bool RdmaQueuePair::IsWinBound(){
	uint64_t w = GetWin();
	/*
	if(sip.Get() == 0x0b00b701 && dip.Get() == 0x0b00c301 && sport == 10000 && dport ==100)
	{
		std::cout << "GetOnTheFly " << GetOnTheFly() << " w " << w << " snd_nxt " << snd_nxt << " snd_una " << snd_una << std::endl;
	}
	*/
    // if(GetOnTheFly() >= 154000)
	// {
	// 	std::cout << std::hex << "0" << sip.Get() << " 0" << dip.Get() << " " ;
	// 	std::cout << std::dec << sport << " " << dport << " snd_nxt " << snd_nxt << " snd_una " << snd_una << " m_pg " << m_pg << std::endl;
	// }
...
uint64_t RdmaQueuePair::HpGetCurWin(){ //hp没用到？？
	if (m_win == 0)
		return 0;
	uint64_t w;
	if (m_var_win){
...
uint16_t RdmaQueuePair::GetExtraStateSize()
{
	return sizeof(m_round) + sizeof(round_seq);
}
...
/*
	...
	m_recoverqueue = CreateObject<DropTailQueue>();
	m_loststate = false;
*/
}
