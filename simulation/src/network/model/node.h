...
/*
  void SetQueue (Ptr<Queue> queue);
	...
	void CopyPacket(Ptr<Packet> packet);
	Ptr<Packet> GetDequeuePacket();
	void EnCopyQueue(Ptr<Packet> p);
*/
...
public:
  ...
  uint32_t data_hash_conflict_number;
  uint32_t NACK_hash_conflict_number;
  uint32_t filter_data_number;
  uint32_t filter_notify_number;
  uint32_t filter_NACK_number;

  uint32_t timeout_number;
  uint32_t seq_expected_number;

  uint32_t udp_lost_number;
  uint32_t ack_lost_number;
  uint32_t last_udp_lost_number;
  uint32_t last_ack_lost_number;
  uint32_t nack_lost_number;
  uint32_t FB_lost_number;
  uint32_t total_lost_number;

  uint32_t udp_find_number;
  uint32_t ack_find_number;
  uint32_t last_udp_find_number;
  uint32_t last_ack_find_number;
  uint32_t nack_find_number;
  uint32_t FB_find_number;
  uint32_t total_find_number;

  uint32_t max_copysize[257];
  uint32_t max_reorderingsize[257];

  uint32_t pfc_times;
  uint32_t lgpause_times;
...
/*
  uint32_t m_counter[257];
  uint32_t m_seqnumber[257];
  uint32_t m_seqparam[257];
  uint16_t aon;
	uint32_t tempparam;
  uint32_t tempseq;
  uint32_t seqcount;
	Ptr<Queue> m_copyqueue;
  uint32_t down_received_but_invaild_highest_seq;
  uint32_t up_has_sent_highest_seq;
...
public:
  ...
  virtual void SendNotifyToSender(uint32_t m_index, uint16_t left, uint16_t right);
  virtual void UpdateReTxReqs(uint32_t m_index, uint16_t left, uint16_t right);
  virtual void EnReorderingQueue(Ptr<Packet> p, uint16_t port);
  virtual uint16_t GeSeqnumber(uint16_t PortIndex);
  virtual uint16_t GetPendingack(uint16_t PortIndex);
  virtual bool ToIsSwitch(uint32_t pidx);
/*
  virtual void CopyPacket(Ptr<Packet> packet);
  virtual Ptr<Packet> GetDequeuePacket();
	virtual void EnCopyQueue(Ptr<Packet> p);
*/
