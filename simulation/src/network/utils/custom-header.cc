...
uint32_t CustomHeader::GetSerializedSize (void) const{
	...
	if (headerType & L4_Header){
		...
		else if (l3Prot == 0xFC || l3Prot == 0xFD) //ack
			len += GetAckSerializedSize();
		else if (l3Prot == 0xFF) //cnp
			len += 8;
		else if (l3Prot == 0xFE) //pfc
			len += 9;
		else if(l3Prot == 0xFB) //notify
			len += 13;
		else if(l3Prot == 0xFA) //notifytoup
			len += 4;
		else if(l3Prot == 0xF9) //empty data
			len += 4;
		else if(l3Prot == 0xF8) //empty ack
			len += 4;
		else if(l3Prot == 0xF7) //lg pause
			len += 3;
	}
	return len;
}
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteHtonU16 (udp.flags);
		  //in-network notification
		  i.WriteHtonU16 (udp.sequence);
		  i.WriteHtonU16 (udp.round);
		  //udp.muh.Serialize(i);
		  udp.ih.Serialize(i);
	  }else if (l3Prot == 0xFF){ // CNP
		  ...
		  i.WriteU16(ack.pg);
		  i.WriteU32(ack.seq);
		  i.WriteU16(ack.sequence);
		  i.WriteU16(ack.latestRxSeqNo);
		  //ack.mah.Serialize(i);
		  udp.ih.Serialize(i);
	  }else if (l3Prot == 0xFE){ // PFC
		  i.WriteU32 (pfc.time);
		  ...
		  i.WriteU8 (pfc.qIndex);
	  	  ...
	  	  i.WriteU16(notify.sport);
		  i.WriteU16(notify.dport);
		  i.WriteU32(notify.m_notifyseq);
		  i.WriteU16(notify.m_round);
		  i.WriteU8(notify.m_pg);
		  i.WriteU16(notify.m_sequence);
	  }else if(l3Prot == 0xFA){ //notifytoup
		  i.WriteU16(notifytoup.m_notifyleft);
		  i.WriteU16(notifytoup.m_notifyright);
	  }else if(l3Prot == 0xF9){ //empty data
		  i.WriteU16(empty.empty_seq);
		  i.WriteU16(empty.pendingack);
	  }else if(l3Prot == 0xF8){ //empty ack
	  	  i.WriteU16(empty.empty_seq);
		  i.WriteU16(empty.pendingack);
	  }else if(l3Prot == 0xF7){
		  i.WriteU16(lgpause.lgtype);
		  i.WriteU8(lgpause.qIndex);
	  }
  }
}
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  udp.flags = i.ReadNtohU16 ();
		  //in-network notification
		  udp.sequence = i.ReadNtohU16 ();
		  udp.round = i.ReadNtohU16 ();
		  //udp.muh.Deserialize(i);
		  if (getInt)
		  ...
		  ack.pg = i.ReadU16();
		  ack.seq = i.ReadU32();
		  ack.sequence = i.ReadU16();
		  ack.latestRxSeqNo = i.ReadU16();
		  //ack.mah.Deserialize(i);
		  if (getInt)
		  ...
		  l4Size = 9;
	  	  ...
	  	  notify.sport = i.ReadU16();
	      notify.dport = i.ReadU16();
		  notify.m_notifyseq = i.ReadU32();
		  notify.m_round = i.ReadU16();
		  notify.m_pg = i.ReadU8();
		  notify.m_sequence = i.ReadU16();
		  l4Size = 13;
	  }else if(l3Prot == 0xFA){ //notifytoup
	  	  notifytoup.m_notifyleft = i.ReadU16();
		  notifytoup.m_notifyright = i.ReadU16();
		  l4Size = 4;
	  }else if(l3Prot == 0xF9){ //empty data
		  empty.empty_seq = i.ReadU16();
		  empty.pendingack = i.ReadU16();
		  l4Size = 4;
	  }else if(l3Prot == 0xF8){ //empty ack
		  empty.empty_seq = i.ReadU16();
		  empty.pendingack = i.ReadU16();
		  ...
		  lgpause.lgtype = i.ReadU16();
		  lgpause.qIndex = i.ReadU8();
		  l4Size = 3;
	  }
  }
...
uint32_t CustomHeader::GetAckSerializedSize(void){
	...
	//1
	//return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) + sizeof(ack.seq) + sizeof(ack.sequence) + IntHeader::GetStaticSize();
	//2
	return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) + sizeof(ack.seq) + sizeof(ack.sequence) + sizeof(ack.latestRxSeqNo) + IntHeader::GetStaticSize();
}

uint32_t CustomHeader::GetUdpHeaderSize(void){
	//in-network notification
	return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.flags) + sizeof(udp.sequence) + sizeof(udp.round) + IntHeader::GetStaticSize();
	//return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.flags) + IntHeader::GetStaticSize();
